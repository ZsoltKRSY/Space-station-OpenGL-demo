...
uniform vec4 fColor; //base color of the fragment
...
float pointSpecularStrength = 0.05f;
float constant = 1.0f;
...
void computePointLight(vec3 lightPos, vec3 lightColor, float ambientStrength, float specularStrength)
{
    ...
    //ambient
    ambient += att * ambientStrength * fColor.rgb;
...
void computeDirLight()
{
    ...
    ambient += dirAmbientStrength * fColor.rgb;
...
void main() 
{
    ...
    if(fColor.a < 1.0f){
        color = transparentFactor * fColor.rgb;

        fragColor = vec4(color, fColor.a);
    }
    else{
        computeDirLight();
        ...
        color = min((ambient + (1.1f - shadow)*diffuse + spotlightDiffuse) * fColor.rgb + (1.0f - shadow)*specular + spotlightSpecular, 1.0f);

        if(isFog)
        {
            float fogFactor = computeFog();

            fragColor = mix(fogColor, vec4(color, 1.0f), fogFactor);
        }
        else
        {
